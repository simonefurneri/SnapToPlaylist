...
export function CreatePlaylistConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-neutral-900/95 border border-neutral-800 rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-2xl relative text-white overflow-hidden animate-in zoom-in-95 duration-300">
        {/* Ambient Glow */}
        ...
        <button
          ...
          onClick={onClose}
          disabled={isCreating}
          className="absolute top-4 right-4 text-neutral-400 hover:text-white p-2 rounded-xl hover:bg-neutral-800 transition-all duration-200 cursor-pointer disabled:opacity-30"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center gap-3.5 mb-5">
          <div className="w-12 h-12 rounded-2xl bg-[#1DB954]/10 border border-[#1DB954]/30 text-[#1DB954] flex items-center justify-center shadow-lg shadow-[#1DB954]/20 flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400">
              Riepilogo delle tracce che verranno aggiunte su Spotify
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-neutral-400 pt-2 border-t border-neutral-850">
            ...
            {notFoundSongs.length > 0 && (
              <span className="text-rose-400 font-semibold flex items-center gap-1 pl-3 border-l border-neutral-800">
                ...
                {notFoundSongs.length} esclusi (non trovati)
              </span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-1.5 mb-6 max-h-48 overflow-y-auto pr-1">
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="font-semibold text-neutral-200 truncate">
                    {song.spotifyTrackName || song.title}
                  </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          <button
            ...
            disabled={isCreating}
            onClick={onClose}
            className="order-2 sm:order-1 flex-1 py-3 px-4 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-xs sm:text-sm font-semibold text-neutral-300 hover:text-white transition-all duration-200 cursor-pointer active:scale-95 disabled:opacity-50"
          >
            Annulla e Modifica
          </button>
          ...
          <button
            ...
            onClick={onConfirm}
            className="order-1 sm:order-2 flex-1 py-3 px-5 rounded-xl bg-gradient-to-r from-[#1DB954] to-emerald-500 hover:from-[#1ed760] hover:to-emerald-400 text-neutral-950 font-extrabold text-xs sm:text-sm transition-all duration-200 shadow-xl shadow-[#1DB954]/25 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 active:scale-95"
          >
            {isCreating ? (
              <>
